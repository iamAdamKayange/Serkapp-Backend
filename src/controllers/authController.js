...
exports.login = async (req, res, next) => {
  ...
  try {
    ...
    // Check database-backed lock status (non-blocking)
    try {
      ...
      console.error('Lock check failed:', lockError.message);
    }
    
    // Check in-memory lock status (non-blocking)
    try {
      ...
      console.error('In-memory lock check failed:', lockError.message);
    }
    ...
    if (!isMatch) {
      ...
      try {
        await trackFailedLogin(email, req.ip || req.connection.remoteAddress, user.id);
      } catch (trackError) {
        console.error('Failed login tracking failed:', trackError.message);
      }
    ...
    try {
      ...
      console.error('Clear failed attempts failed:', clearError.message);
    }
