...
class HouseImage {
  ...
  static async create(houseId, imageUrl, publicId, displayOrder = null) {
    const order = displayOrder !== null ? displayOrder : await this.getNextOrder(houseId);
    const result = await pool.query(
      `INSERT INTO house_images (house_id, image_url, public_id, display_order)
       VALUES ($1, $2, $3, $4) RETURNING *`,
      [houseId, imageUrl, publicId, order]
    );
  ...
  static async findById(id) {
    const result = await pool.query(`SELECT * FROM house_images WHERE id = $1`, [id]);
    return result.rows.length ? new HouseImage(result.rows[0]) : null;
  }
